...
- **Carried-state layout is canonical and must be parity-gated.** The DeltaNet
  recurrent state's global layout is `state[(vh·V + vd)·K + kd]` = [v_heads, V, K]
  (defined by the sequential `deltanet_decode` kernel; the chunked prefill kernel
  transposes at its global↔SMEM boundary to honour it). Any pair of kernels that hands
  a carried state across implementations (prefill↔decode, graph↔eager,
  interpreter↔host) needs an explicit carried-state parity gate **with a nonzero
  initial state** — output-only gates cannot see state-layout bugs (the 2026-06-10
  decode-vs-prefill divergence shipped exactly this way; see `DAILY.md`). The smoke
  chunked-vs-sequential case gates the DeltaNet pair; do not weaken its nonzero
  initial state.
- **max_context is opt-in beyond the default.** `EngineConfig::default().max_context`
  is `DEFAULT_MAX_CONTEXT` (16384); the model ceiling (262144) is reachable only by
  explicitly setting `EngineConfig.max_context` / `--max-context` /
  `QWEN36_MAX_CONTEXT`. Engine constructors validate against the topology's
  `max_position_embeddings` — keep the explicit error, never clamp silently.
...
1. **Correctness outranks throughput.** A known correctness bug takes priority over any
   perf work; never trade parity for tok/s. (Precedent: the decode-vs-prefill logits
   divergence was P0 until fixed on 2026-06-10; `qwen36 decode-vs-prefill-check` is now
   its regression gate.)
