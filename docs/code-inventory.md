...
| `QWEN36_LM_HEAD_FP8` | **on** (2026-06-11 nuit) | quantizes lm_head to e4m3 per-row at init and DROPS the BF16 original (−1.18 GiB). N=1 GEMV 785 µs = 90% peak (cuBLAS BF16: 1.65 ms); batched verify N=5 1.78 ms (cuBLAS: 2.2). MTP=4 @3K cycle −6.2 ms (+7.6% pure), MTP=0 +1.8%, floor 10/10, DFlash AL unchanged (drafter keeps a BF16 lm_head via `EngineConfig.keep_bf16_lm_head`). Accepted artifact costs: raw-corpus @128 acceptance −0.12 and synthetic full-accept gate 95.3 → ~90 (low-margin argmax flips; production regimes clean, auto-fallback bounds the rest). `=0` restores BF16 |
