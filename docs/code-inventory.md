...
| DeltaNet decode + chunked prefill | `kernels-cuda/deltanet.cu`, `deltanet_prefill.cu` | 48 linear-attention layers; chunked prefill default ON. **Tiny-chunk fast path (2026-06-12, default ON)**: T≤16 chunks (speculative verify) bound the C=32 phases to one 16-wide tile + forward-sub bounded to `valid` — BIT-EXACT by construction (memcmp gate, 11 T values), kernel −40% median at T=5. Kill: `QWEN36_DELTANET_TINY_CHUNK=0`; hook `qwen36_deltanet_prefill_set_tiny` |
...
| lm_head argmax exact deux étages — **v2 top-8 rescore** (scan FP8 **ou FP4** → top-8 candidates → re-score FP64 vs BF16 → garde vs max(9e, max v2-blocs) → fallback complet prédiqué) | `QWEN36_LM_HEAD_FP8_MARGIN=<ε>` (**reco 0.5 en fp8, 1.2 en fp4**) ; `QWEN36_LM_HEAD_SCAN=fp4` (scan NVFP4 : −2.2 ms/cycle vs fp8 à trajectoire égale, store 0.71 au lieu de 1.27 GiB ; sonde `lmhead_fp4_probe.py` rang 0/28) ; debug : `QWEN36_LM_HEAD_MARGIN_SCOPE=top2` (v1), `=rows1` (bisection) | `kernels-cuda/lm_head_fp8.cu` (`qwen36_lm_head_top8_rescore`, `qwen36_lm_head_top2_margin`), `ops.cu` (skip_flags), `engine.rs` (`lm_head_two_stage_argmax_rows`) | **SHIPPED opt-in (2026-06-12)** : @128 acc bit-identique (16 déc.) + decode 39.6→48.2 (+21.7%, cycle −13 ms) ; fallbacks 42% (v1) → ~6% (v2) ; @8K/24K = trajectoires-loterie connues (flips bornés aux ~6% de gardes échouées → re-score PR #11). Défaut ON différé à la validation grille de régimes. JSON bench : `lm_head_margin_fallbacks`. Gates smoke : top8 (5/5 certifiés, parité host-FP64), top2+marge, GEMV FP8 n=5 batched-vs-single bit-exact |
...
- ~~**Per-token decode vs prefill logits divergence**~~ — **FIXED 2026-06-10**: the chunked DeltaNet prefill kernel wrote its final recurrent state transposed ([K,V] instead of the canonical [v_heads,V,K] the sequential decode kernel reads). `decode-vs-prefill-check` now reports argmax_match=true, cos ≥ 0.997; gated by the chunked-vs-sequential output+state smoke case. See `DAILY.md` § 2026-06-10.
...
- ~~**Default config OOM trap**~~ — **FIXED 2026-06-10**: `EngineConfig::default().max_context` is now 16384 (`DEFAULT_MAX_CONTEXT`); 262K is explicit opt-in (`EngineConfig.max_context`, `--max-context`, or `QWEN36_MAX_CONTEXT`), validated against the model ceiling at engine construction. The fused-stores-vs-KV squeeze at intermediate contexts on a busy GPU still exists — `QWEN36_LONG_CONTEXT_MODE=1` remains the workaround there.
...
| `QWEN36_MAX_CONTEXT` | 16384 | `EngineConfig::default()` max_context; 1..=262144, strictly parsed (garbage panics). 262K context is this explicit opt-in (or set `max_context` directly) |
...
| `QWEN36_DELTANET_SEQ_SHORT_CHUNK` | **off** | `=1` routes ≤8-token chunks (speculative verify windows) to sequential DeltaNet — kernel win re-measured 2026-06-12 (31.9→43.5 tok/s @128) but acceptance decays MONOTONICALLY with ctx (+0.11 @128 / −0.15 @3K / −0.23 @8K): algorithmic WY↔sequential state drift compounds, NOT trajectory lottery, NOT bf16 rounding cadence (fp32-resident variant only −14% mean drift). Verify must share the prefill algorithm; fast-path chunked T≤16 bit-exact is the open lever |
| `QWEN36_DELTANET_RESIDENT` | **on** (when eligible) | `=0` disables the FP32-resident multi-token deltanet_decode variant (state in FP32 SMEM across the token loop, single bf16 round; gated, tokens>1, K=V=128 only — the captured 1-token decode graph never routes here). Strictly better drift (−14% mean) than the per-token-rounding body on the SEQ_SHORT_CHUNK path; test hook `qwen36_deltanet_set_resident` (2026-06-12) |
...
| `decode-vs-prefill-check` | decode/prefill consistency regression check (argmax + logits cos; the 2026-06-10 DeltaNet state-layout fix is gated by it) | yes |
