...
| MTP fused lm_head matvec+argmax | `QWEN36_MTP_FUSED_ARGMAX=1` | `kernels-cuda/ops.cu` (`qwen36_bf16_matvec_argmax_rows`) | +4 à +6.6 ms/cycle MTP=4 @128 vs cuBLAS matvec + argmax séparés, acceptance inchangée @128 (2026-06-11, PR #11 mergée en opt-in). **BUG LATENT (2026-06-12) : PAS argmax-équivalent à ctx 8192** — acc 0.55→0.4114583333333333, même trajectoire que le mode marge (sites d'appel partagés) → un site device-argmax lit une mauvaise ligne/buffer à long ctx ; chasse en cours |
| lm_head argmax exact deux étages (scan FP8 + garde de marge + re-score BF16 prédiqué) | `QWEN36_LM_HEAD_FP8_MARGIN=<ε>` (ex. 1.0) ; debug `QWEN36_LM_HEAD_MARGIN_SCOPE=rows1` | `kernels-cuda/lm_head_fp8.cu` (`qwen36_lm_head_top2_margin`), `ops.cu` (skip_flags), `engine.rs` (`lm_head_two_stage_argmax_rows`) | WIP (2026-06-12) : bit-exact @128 (acc identique 16 déc., +3.2% decode, fallback ~42% — marges tête MTP serrées) ; @8K bloqué par le bug latent PR #11 ci-dessus (innocenté par bisection 3 voies). v2 scopée : re-score top-8 plat. JSON bench : `lm_head_margin_fallbacks`. Gates smoke : top2+marge, GEMV FP8 n=5 batched-vs-single bit-exact |
