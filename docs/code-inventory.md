...
| lm_head argmax exact deux étages (scan FP8 + garde de marge + re-score BF16 prédiqué) | `QWEN36_LM_HEAD_FP8_MARGIN=<ε>` (ex. 1.0) ; debug `QWEN36_LM_HEAD_MARGIN_SCOPE=rows1` | `kernels-cuda/lm_head_fp8.cu` (`qwen36_lm_head_top2_margin`), `ops.cu` (skip_flags), `engine.rs` (`lm_head_two_stage_argmax_rows`) | WIP (2026-06-12) : bit-exact vs default tant qu'aucune ligne ne tombe sous la garde (marge ≥ ε) — les lignes garde-échouée héritent du flip near-tie du kernel de re-score (classe verify-perturbation, cf. ligne PR #11). @128 acc identique 16 déc., +3.2% decode, fallback ~42%. Gate re-cadré : neutralité d'acceptance EN ESPÉRANCE sur la grille (mesure en cours) + v2 top-8 plat pour la perf. JSON bench : `lm_head_margin_fallbacks`. Gates smoke : top2+marge, GEMV FP8 n=5 batched-vs-single bit-exact |
